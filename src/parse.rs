...
fn parse_declare(input: &str) -> IResult<&str, Expr> {
...
fn parse_call(input: &str) -> IResult<&str, Expr> {
    let parse_arg = delimited(tag("("), parse_constant, tag(")"));
    let parser = (parse_name, parse_arg);
    map(parser, |(name, arg)| Expr::Call {
        name,
        value: Box::new(arg),
    })
    .parse(input)
...
fn parse_function(input: &str) -> IResult<&str, Expr> {
    ...
    let parser = preceded(tag("fn"), (ws(parse_name), parse_arg, ws(parse_body)));
    map(parser, |(name, arg, body)| Expr::Function {
        name,
        arg: Box::new(arg),
        body,
    })
